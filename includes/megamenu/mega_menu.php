...
class Ideapark_Mega_Menu {
	...
	public $_version;
	public $_token;
	private $fields = [ 'columns', 'content', 'icon', 'html_block', 'product_category', 'product_attr', 'expand' ];
	private $columns = 1;
	private $expand = 0;
	private $counter = 0;
			...
			<p class="ip-field-custom ip-field-custom--depth-0 ip-field-custom--<?php echo esc_attr( $field ); ?>"
			   ...
			   data-item-id="<?php echo esc_attr( $item_id ); ?>">
				<label>
					...
					<?php if ($field == 'html_block') { ?><div><a href="<?php echo esc_url( admin_url( 'edit.php?post_type=html_block' ) ); ?>"><?php esc_html_e( 'Manage html blocks', 'luchiana' ) ?></a></div><?php } ?>
				</span>
				</label>
			</p>
		<?php } ?>
