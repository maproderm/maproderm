...
<form class="cart grouped_form"
	...
	<table cellspacing="0" class="woocommerce-grouped-product-list group_table">
		...
		$previous_post           = $post;
		$grouped_product_columns = apply_filters( 'woocommerce_grouped_product_columns', [
			'label',
			'price',
			'quantity',
		], $product );
			...
			foreach ( $grouped_product_columns as $column_id ) {
				...
				switch ( $column_id ) {
					...
					case 'label':
						...
						$value .= '</label>';
						if ( in_array( 'price', $grouped_product_columns ) ) {
							$price = $grouped_product_child->get_price_html() . wc_get_stock_html( $grouped_product_child );
							$value .= ideapark_wrap(
								apply_filters( 'woocommerce_grouped_product_list_column_' . 'price', $price, $grouped_product_child ),
