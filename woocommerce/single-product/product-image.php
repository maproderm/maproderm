...
<?php if ( $images ) {
	foreach ( $images as $i => $image ) {
		...
		if ( ! empty( $image['video_url'] ) ) {
			if ( ideapark_mod( 'shop_product_modal' ) ) {
				$image_wrap_open  = sprintf( '<a download href="%s" class="c-product__image-link %s" data-index="%s" data-product-id="%s" data-elementor-open-lightbox="no" onclick="return false;">', esc_url( $image['video_url'] ), implode( ' ', $link_class ), $index ++, $post->ID );
				$image_wrap_close = '</a>';
			...
			if ( ideapark_mod( 'shop_product_modal' ) ) {
				$image_wrap_open  = sprintf( '<a download href="%s" class="c-product__image-link %s" data-size="%sx%s" data-index="%s" data-product-id="%s" data-elementor-open-lightbox="no" onclick="return false;">', esc_url( $image['full'][0] ), implode( ' ', $link_class ), intval( $image['full'][1] ), intval( $image['full'][2] ), $index ++, $post->ID );
				$image_wrap_close = '<span class="c-product__loading js-loading-wrap"></span></a>';
			} elseif ( ideapark_mod( '_is_quickview' ) ) {
				$image_wrap_open  = sprintf( '<a href="%s" class="c-product__image-link %s" data-elementor-open-lightbox="no">', esc_url( get_the_permalink() ), implode( ' ', $link_class ) );
				$image_wrap_close = '</a>';
			} else {
				$image_wrap_open  = '';
				$image_wrap_close = '';
			...
			echo apply_filters( 'woocommerce_single_product_image_html', sprintf( '<div class="c-product__slider-item ' . ( ideapark_mod( 'shop_product_modal' ) ? 'c-product__slider-item--zoom' : '' ) . ' woocommerce-product-gallery__image ">%s%s%s</div>', $image_wrap_open, $image['image'], $image_wrap_close ), $post->ID );
		}
	}
} else {
	echo apply_filters( 'woocommerce_single_product_image_html', sprintf( '<div class="c-product__slider-item"><img src="%s" alt="%s" /></div>', wc_placeholder_img_src( 'woocommerce_single' ), esc_attr__( 'Placeholder', 'woocommerce' ) ), $post->ID );
} ?>
...
<?php } else { ?>
	...
	<div
		class="c-product__slider c-product__slider--carousel h-carousel h-carousel--inner h-carousel--hover h-carousel--dots-hide <?php ideapark_class( in_array( ideapark_mod( 'product_page_layout' ), [
			'layout-3',
			'layout-4'
				...
				<?php foreach ( $images as $ii => $image ) { ?>
					<?php echo apply_filters( 'woocommerce_single_product_image_thumbnail_html',
						sprintf( '<div class="c-product__thumbs-item ' . ( ! $ii ? 'active' : '' ) . '"><button type="button" class="h-cb js-single-product-thumb %s" data-index="%s" %s>%s</button></div>',
							...
							! empty( $image['thumb_url'] ) ? ( ! empty( $image['is_youtube_preview'] ) ? '<span class="c-product__thumbs-video-mask"></span>' : '' ) . '<i class="c-play"></i>' : $image['thumb']
